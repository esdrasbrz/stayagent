...
from app.models import SearchRequest, JobStatus, StayResult, JobResultResponse, JobStateEnum
...
async def crawl_background_task(operation_id: str, request: SearchRequest, store: JobStore):
    try:
...
async def start_search(request: SearchRequest, store: JobStore = Depends(get_job_store)):
    ...
    task = asyncio.create_task(crawl_background_task(operation_id, request, store))
...
async def get_search_status(operation_id: str, store: JobStore = Depends(get_job_store)):
    status = await store.get_job_status(operation_id)
    if not status:
...
async def cancel_search(operation_id: str, store: JobStore = Depends(get_job_store)):
    ...
    if status.status in [JobStateEnum.COMPLETED, JobStateEnum.FAILED, JobStateEnum.CANCELLED]:
...
async def get_search_results(operation_id: str, store: JobStore = Depends(get_job_store)):
    status = await store.get_job_status(operation_id)
    if not status:
        raise HTTPException(status_code=404, detail="Operation ID not found")
